...
//Three main arrays to do shit with
cl_mem gpu_a, gpu_b, gpu_c;
cl_int status;
//End of Initializations

bool init_opencl();
bool allocate_mem(int num_of_elements);

//Main methods

int fpga_sort(int num_of_elements, float *data)
{
    printf("What's up people\n");

    float *a, *b;
    a = (float *)malloc(num_of_elements*sizeof(cl_float));
    for (int i = 0; i < num_of_elements; i++)
      a[i] = (cl_float)data[i];
    b = (float *)malloc(num_of_elements*sizeof(cl_float));

    printf("Trying to init_opencl\n");
    init_opencl();
    

    printf("Trying to allocate_mem\n");
    allocate_mem(num_of_elements);

    status = clEnqueueWriteBuffer(queue[0], gpu_a, CL_FALSE, 0,
     num_of_elements*sizeof(float), a, 0, NULL, NULL);
    if (status != CL_SUCCESS)
    {
      fprintf(stderr, "Failed to transfer buffer for output");
      exit(1);
    }

    //status = clEnqueueWriteBuffer(queue[0], gpu_b, CL_FALSE, 0,
    //num_of_elements*sizeof(float), b, 0, NULL, NULL);
    //if (status != CL_SUCCESS)
    //{
      //fprintf(stderr, "Failed to transfer buffer for output");
      //exit(1);
    ...
    status = clSetKernelArg(kernel[0], argi++, sizeof(cl_mem), &gpu_a);
    checkError(status, "Failed to set argument %d", argi - 1);

    //status = clSetKernelArg(kernel[0], argi++, sizeof(cl_mem), &gpu_b);
    //checkError(status, "Failed to set argument %d", argi - 1);
    
    status = clSetKernelArg(kernel[0], argi++, sizeof(num_of_elements), &num_of_elements);
    checkError(status, "Failed to set argument %d", argi - 1);

    size_t max_work_size[1];
    ...
    status = clEnqueueReadBuffer(queue[0], gpu_b, CL_TRUE,
      0, num_of_elements*sizeof(float), b, 0, NULL, NULL);
    checkError(status, "Failed to read output sorted array");
...
bool allocate_mem(int num_of_elements) {
  ...
  gpu_b = clCreateBuffer(context, CL_MEM_WRITE_ONLY,
    num_of_elements * sizeof(float), NULL, &status);
  if (status != CL_SUCCESS)
  {
    fprintf(stderr, "Failed to create buffer for input B");
...
printf("Device.reset(getDevices())");
